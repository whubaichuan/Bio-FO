...
def train(args, device, train_loader, traintest_loader, test_loader):
    ...
    for trial in range(1,args.trials+1):
        ...
        store_num = 5 #chbmit, cifar10, cifar100:5, mnist, mitbih:6
        error_mnist = np.zeros((2,store_num*100))
        gap=100  #chbmit:10, mnist, cifar10, cifar100:100, mitbih:146

        for epoch in range(1, args.epochs + 1):
            # Training
            error_mnist = train_epoch(args, model, device, train_loader, optimizer, loss,epoch,store_num,error_mnist,gap,traintest_loader,test_loader)

            # Compute accuracy on training and testing set
            print("\nSummary of epoch %d:" % (epoch))


        np.save(os.path.split(os.path.realpath(__file__))[0]+"/error_cifar100_drtp.npy",error_mnist)

def train_epoch(args, model, device, train_loader, optimizer, loss,epoch,store_num,error_mnist,gap,traintest_loader,test_loader):
    model.train()
    ...
    for batch_idx, (data, label) in enumerate(tqdm(train_loader)):
        ...
        if (batch_idx+1) % gap == 0:
            train_error=test_epoch(args, model, device, traintest_loader, loss, 'Train')
            test_error=test_epoch(args, model, device, test_loader, loss, 'Test')
            
            error_mnist[0,(epoch-1)*store_num+int((batch_idx+1) / gap)-1] = train_error
            error_mnist[1,(epoch-1)*store_num+int((batch_idx+1) / gap)-1] = test_error
    return error_mnist
