...
class FF_imagenet(torch.utils.data.Dataset):
    def __init__(self, opt, partition, num_classes=100):
        self.opt = opt
        self.mnist = utils.get_imagenet_partition(opt, partition)
...
class FF_CHBMIT(torch.utils.data.Dataset):
    def __init__(self, opt, partition, num_classes=2):
        self.opt = opt
        self.mnist = utils.get_CHBMIT_partition(opt, partition)
        self.num_classes = num_classes
        self.uniform_label = torch.ones(self.num_classes) / self.num_classes
    ...
    def _get_pos_sample(self, sample, class_label):
        ...
        pos_sample = sample.clone()
        pos_sample[0, 0, : self.num_classes] = one_hot_label*self.opt.input.label_weight*torch.max(sample) #caution: need to change based on the dataset
        return pos_sample
    ...
    def _get_neg_sample(self, sample, class_label):
        ...
        neg_sample = sample.clone()
        neg_sample[0, 0, : self.num_classes] = one_hot_label*self.opt.input.label_weight*torch.max(sample) #caution: need to change based on the dataset
        return neg_sample
    ...
    def _get_all_sample(self, sample):
        ...
        for i in range(self.num_classes):
            ...
            one_hot_label = torch.nn.functional.one_hot(
            torch.tensor(i), num_classes=self.num_classes)
            all_samples[i, 0, 0, : self.num_classes] = one_hot_label.clone()*self.opt.input.label_weight*torch.max(sample) #caution: need to change based on the dataset
        return all_samples
